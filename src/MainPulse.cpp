...
void func_parr3(int nb_obps, int idx,  string instpath, int source, int sink, double demandPCT){
	...
	cout << source << '-' << sink << ' ' << test_._status << '?' << test_._curbest_objval << " = ";
	for(unsigned i=0; i <test_._curbest_path.size(); i++)
		cout << test_._curbest_path[i] << ' ';
	cout << '\n';
...
int main(int argc, const char* argv[]) {
	...
	func_parr3(nb_obps, idx, "/home/cai/Dropbox/Box_Research/Github/RRARP_LinKern/dat/Inst_Pulse/", 0, 1, demand_pct);
	return 0;
}
